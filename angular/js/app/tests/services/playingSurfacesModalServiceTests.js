describe('playingSurfacesModalServiceTests Test Suite', function(){

    var playingSurfacesModalService, httpBackend;
    ...
    beforeEach(inject(function(_playingSurfacesModalService_, $httpBackend){
        playingSurfacesModalService = _playingSurfacesModalService_;
    ...
    it('should confirm that all AJAX requests return a resolved promise if APIs return a successful response', function(){
		// Test GET endpoint
        var getSurfacesResult = playingSurfacesModalService.getSurfaces();
        ...
        // Test PUT endpoint
        var updateSelectedSurfacesResult = playingSurfacesModalService.updateSelectedSurfaces();
        httpBackend.when('PUT', /\/event/).respond({
            myKey: 'myString'
        });
        httpBackend.flush();
        expect(updateSelectedSurfacesResult.$$state.value).toEqual({
            myKey: 'myString'
        });
    });
    ...
    it('should confirm that all AJAX requests return a promise call failed message if APIs return a failure', function(){
        ...
        httpBackend.flush();
        expect(updateSelectedSurfacesResult.$$state.value).toEqual('chicken fingers');
    });
