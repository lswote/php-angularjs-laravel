describe('openTimeSlotsModalService Test Suite', function(){

    var openTimeSlotsModalService, httpBackend;
    ...
    beforeEach(inject(function(_openTimeSlotsModalService_, $httpBackend){
        openTimeSlotsModalService = _openTimeSlotsModalService_;
    ...
    it('should confirm that all AJAX requests return a resolved promise if APIs return a successful response', function(){
		// Test GET endpoint
        var getOpenTimeSlotsInfoResult = openTimeSlotsModalService.getOpenTimeSlotsInfo();
        httpBackend.when('GET', /\/event/).respond({
            myKey: 'myString'
        });
        httpBackend.flush();
        expect(getOpenTimeSlotsInfoResult.$$state.value).toEqual({
            myKey: 'myString'
        });
    });
    ...
    it('should confirm that all AJAX requests return a promise call failed message if APIs return a failure', function(){
        ...
        httpBackend.flush();
        expect(getOpenTimeSlotsInfoResult.$$state.value).toEqual('chicken fingers');
    });
