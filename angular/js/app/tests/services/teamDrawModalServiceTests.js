describe('teamDrawModalService Test Suite', function(){

    var teamDrawModalService, httpBackend;
    ...
    beforeEach(inject(function(_teamDrawModalService_, $httpBackend){
        teamDrawModalService = _teamDrawModalService_;
    ...
    it('should confirm that all AJAX requests return a resolved promise if APIs return a successful response', function(){
    	// Test GET endpoint
    	var getNextUnassignedGroupResult = teamDrawModalService.getNextUnassignedGroup();
        httpBackend.when('GET', /\/event/).respond({
            myKey: 'myString'
        });
        httpBackend.flush();
        expect(getNextUnassignedGroupResult.$$state.value).toEqual({
            myKey: 'myString'
        });
    });
    ...
    it('should confirm that all AJAX requests return a promise call failed message if APIs return a failure', function(){
        ...
        httpBackend.flush();
        expect(getNextUnassignedGroupResult.$$state.value).toEqual('chicken fingers');
    });
