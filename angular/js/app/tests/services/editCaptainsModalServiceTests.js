describe('editCaptainsModalService Test Suite', function(){

    var editCaptainsModalService, httpBackend, helperService;
    ...
    beforeEach(inject(function(_editCaptainsModalService_, $httpBackend){
        editCaptainsModalService = _editCaptainsModalService_;
    ...
    it('should confirm that all AJAX requests return a resolved promise if APIs return a successful response', function(){
    	// Test PUT endpoint
        var updateEventTeamCaptainsResult = editCaptainsModalService.updateEventTeamCaptains();
        httpBackend.when('PUT', /\/event/).respond({
            myKey: 'myString'
        });
        httpBackend.flush();
        expect(updateEventTeamCaptainsResult.$$state.value).toEqual({
            myKey: 'myString'
        });
    });
    ...
    it('should confirm that all AJAX requests return a promise call failed message if APIs return a failure', function(){
        ...
        httpBackend.flush();
        expect(updateEventTeamCaptainsResult.$$state.value).toEqual('chicken fingers');
    });
