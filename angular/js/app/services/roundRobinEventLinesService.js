teamsRIt.service('roundRobinEventLinesService', function(){
		...
		if(standardTeamCounts.indexOf(teamCount) > -1){
			if(eventConfig.combPlay !== null && (eventConfig.singleWomenLines || eventConfig.singleWomenLines === 0) &&
			   (eventConfig.singleMenLines || eventConfig.singleMenLines === 0)){
				...
				var additionalWomen = (femaleCount % teamCount) + (extraWomenPerTeam * teamCount - combLines);
				var additionalMen = (maleCount % teamCount) + (extraMenPerTeam * teamCount - combLines);
	...
	this.calculateLinesGenderSinglesOnlyPerTeam = function(eventConfing, femaleCount, maleCount, teamCount, womenSitting, menSitting){
		...
		return {
			totalTeams: teamCount,
			womenLines: womenLines ? womenLines : null,
			menLines: menLines ? menLines : null,
	...
	this.calculateLinesMixedPerTeam = function(eventConfing, femaleCount, maleCount, teamCount, womenSitting, menSitting){
		if(standardTeamCounts.indexOf(teamCount) > -1){
			...
			var additionalWomen = (femaleCount % teamCount) + ((womenPerTeam - doubleLines) * teamCount);
			var additionalMen = (maleCount % teamCount) + ((menPerTeam - doubleLines) * teamCount);
			var additionalPlayers = additionalWomen + additionalMen;
		}
		return {
			totalTeams: teamCount,
	...
	this.calculateLinesMixedSinglesOnlyPerTeam = function(eventConfing, femaleCount, maleCount, teamCount, womenSitting, menSitting){
		if(standardTeamCounts.indexOf(teamCount) > -1){
			...
			var additionalWomen = (femaleCount % teamCount) + ((womenPerTeam - womenHalfOfMixedSingles) * teamCount);
			var additionalMen = (maleCount % teamCount) + ((menPerTeam - menHalfOfMixedSingles) * teamCount);
			var additionalPlayers = additionalWomen + additionalMen;
		}
		return {
			totalTeams: teamCount,
